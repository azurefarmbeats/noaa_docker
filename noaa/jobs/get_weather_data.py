...
class GetWeatherDataJob:
    ...
    def __get_weather_data_location_id(self):
        ...
        weather_data_location_payload["name"] = "NOAA_ISD_job_generated_location_[" + str(FLAGS.latitude) + "," + str(FLAGS.longitude) + "]" 
...
def main(argv):
    try:
        ...
        # get weather data
        job.get_weather_data(start_date=FLAGS.from, end_date=FLAGS.to, lat=float(FLAGS.latitude), lon=float(FLAGS.longitude))
    except Exception as err:
        if FLAGS.job_status_blob_sas_url:
